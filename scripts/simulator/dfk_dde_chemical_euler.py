...
def solve_dde_euler(dde_model, init_condition, times, fargs=None):

    curr_state = ddeVar(init_condition, times[0])

    last_state = curr_state(times[0])
    states_history = [last_state]

    num_steps = len(times)

    def temp_var(ddevar_old, next_val, t_next):
        def get_val(t):
            if t >= t_next:
                return next_val
            else:
    ...
    for s in range(1, num_steps):
        dt = times[s]-times[s-1]

        deriv = dfk_dde_model_1D_diffusion(curr_state, times[s-1], *fargs)
        next_state_guess = last_state + (dt)*deriv
        deriv = dfk_dde_model_1D_generation(temp_var(curr_state,next_state_guess, times[s-1]), times[s-1], *fargs)
        next_state_guess = next_state_guess + (dt)*deriv

        # deriv_next = dde_model(temp_var(curr_state, next_state_guess, times[s]), times[s], *fargs)
        # next_state = last_state + (dt)*(deriv+deriv_next)/2.0
        next_state = next_state_guess

        last_state = next_state
        curr_state.update(times[s], last_state)

        states_history.append(last_state)
...
def dfk_dde_model(
    ...
    diff_t_chem[1:] = (
        ...
        + chem_rate * (rho_cell_total[1:]/rho_0)*(4.-rho_chem/rho_0)
    )
    ...
    diff_t_chem[0] = (
        ...
        # Cells secreting the chemical
        + chem_rate * (rho_cell_total[0]/rho_0)*(4.-rho_chem/rho_0)
    )
...
def dfk_dde_model_1D(
    ...
    gradient_D = get_nabla_reflecting(r_vals, D)
    print("rho_g =", rho_g)
    print("rho_chem =", rho_chem)
    print("laplace_g =", laplace_g)
    print("D_g =", D)

    diff_t_g = (
        (D) * (laplace_g)
        # Deal with D being non-constant
        + (gradient_D) * (gradient_g)
        # The g-state cells entering the s-phase
        - enter_division
        # The previous s-phase cells now proliferating
        + 2 * enter_division_d
    ...
    diff_t_chem = (
        (D_chem) * (laplace_chem)
        # Cells secreting the chemical
        + chem_rate * (rho_cell_total/rho_0)*(4-rho_chem)
    )
...
def dfk_dde_model_1D_diffusion(
    Y,
    t,
    Delta_t_s,
    D_base,
...
def dfk_dde_model_1D_generation(
    Y,
    t,
    Delta_t_s,
    D_base,
    ...
    # Calculate spatially dependent diffusion
    diff_t_g = 2 * enter_division_d - enter_division

    # chalculate chemical diffusion at all but origin
    rho_cell_total = rho_g + rho_s
    diff_t_chem = (
        # Cells secreting the chemical
        + chem_rate * (rho_cell_total/rho_0)*(1.-rho_chem/1.0)
    )
...
if __name__ == "__main__":
    ...
    def smooth_init_condition(t):
        ...
        def faux_gaussian(x, mu, sig):
            exp_val = np.exp(-(x - mu) / sig)
        ...
        lower_cutoff = R_i * 0.5
        upper_cutoff = R_i * 1.5
        ...
        curve = gaussian(r_vals[r_filter], 0.0, R_i)
        rho_g[r_filter] = d_i *curve/np.max(curve) # d_i * (1 - ((r_vals[r_filter]-lower_cutoff)/dist)
                          #       ** 0.5) * ((upper_cutoff-r_vals[r_filter])/dist)**0.5
