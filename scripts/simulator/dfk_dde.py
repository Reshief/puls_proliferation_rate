...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=1,
        type=float,
        help="The effective diffusion constant with which the tissue spreads.",
    )
    ...
    def rect_init_condition(t):
        ...
        return np.concatenate((rho_g, rho_s))
    ...
    with open(output_prefix + "full_trajectory.txt", "w") as trajectory_out:
        ...
        trajectory_out.write(
            "# First line: -1 then tab-separated list of r coordinates once for \\rho_g(r) and once for \\rho_s(r). (total of 2n+1 entries)\n"
        )
        trajectory_out.write(
            "# Then after that each line has 2n+1 entries until the end of the file with the format:\n"
        )
        trajectory_out.write(
            "# <time t>\t<n tab-separated values of \\rho_g(r)>\t<n tab-separated values of \\rho_s(r)>\n"
        )
